...
public class Login implements ActionListener {
    ...
    if (!email_username.matches("^[A-Za-z0-9._%+-]+@[A-Za-z0-9.-]+\\.[A-Za-z]{2,6}$")) {
      ...
      return;
        ...
        try {
          ...
          e.printStackTrace();
          JOptionPane.showMessageDialog(frame, "Something went wrong while reading from Database.", "Error", JOptionPane.ERROR_MESSAGE);
      }
    }
  ...
  private boolean checkCredentials(String file_path, String email_username, String password) throws IOException { // throws IOException for jodi kono error paoya jai show korbe
    List<String> lines = Files.readAllLines(Paths.get(file_path));
    for (int i = 0; i < lines.size(); i++) {
      if ((lines.get(i).contains("Username: " + email_username) || lines.get(i).contains("Email: " + email_username))
          && lines.get(i + 1).contains("Password: " + password)) {
        return true;
      }
    }
    return false;
  ...
  public void actionPerformed(ActionEvent e) {
    ...
    if ((e.getSource()).equals(Registration)) {
      ...
      new core.connect(); // move to registration folder
    }
